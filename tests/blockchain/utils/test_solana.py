...
class TestSolanaGetRequest(TestSolana):
    ...
    def test_get_request_with_wrong_api(self):
        ...
        with self.assertRaises(RequestException) as e:
            self.base_chain._get_request(self.base_chain.host,
                                         self.base_chain.params,
                                         self.base_chain.headers)

        self.assertTrue(e.exception.args[0]['error'])
    ...
    def test_get_request_with_wrong_host(self):
        ...
        # self.assertIn('Expecting value', result['error'])
        with self.assertRaises(RequestException) as e:
            self.base_chain._get_request('https://public-api.solscan.io/wrong',
                                         self.base_chain.params,
                                         self.base_chain.headers)
